"""
Custom exceptions for Memoria with comprehensive error handling
"""
...
class MemoriaError(Exception):
    """Base exception for Memoria with enhanced error context"""
...
class DatabaseError(MemoriaError):
...
class ValidationError(MemoriaError):
...
class IntegrationError(MemoriaError):
...
class AuthenticationError(MemoriaError):
...
class SecurityError(MemoriaError):
...
class RateLimitError(MemoriaError):
...
class ProcessingError(MemoriaError):
...
class TimeoutError(MemoriaError):
...
class ResourceExhaustedError(MemoriaError):
...
class ExceptionHandler:
    ...
    @staticmethod
    def log_exception(exception: MemoriaError, logger=None) -> None:
        ...
        ).error("Exception occurred: {}", exception.error_code)
