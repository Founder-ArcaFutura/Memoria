"""
Security audit service for Memoria database operations
...
class DatabaseSecurityAuditor:
    ...
    def _record_findings(self, new_findings: list[SecurityFinding]) -> None:
        """Persist new findings while preventing unbounded growth."""

        if not new_findings:
            return

        existing_keys = {self._finding_key(finding) for finding in self.findings}
        ...
        if len(self.findings) > self.MAX_FINDINGS_RETAINED:
            self.findings = self.findings[-self.MAX_FINDINGS_RETAINED :]

    @staticmethod
    def _finding_key(finding: SecurityFinding) -> tuple[Any, ...]:
        """Create a hashable key for deduplicating findings."""
