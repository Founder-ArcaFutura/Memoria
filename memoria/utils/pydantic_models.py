...
class AgentPermissions(BaseModel):
    """Simple permission model for agents"""

    can_read: bool = True
    can_write: bool = True
    can_edit: bool = True
    # Weighting for search ranking components
...
# Define constrained types using Pydantic constrained types
ConfidenceScore = confloat(ge=0.0, le=1.0)
ImportanceScore = confloat(ge=0.0, le=1.0)
RelevanceScore = confloat(ge=0.0, le=1.0)
PriorityLevel = conint(ge=1, le=10)
...
class ProcessedMemory(BaseModel):
    ...
    x_coord: float | None = Field(default=None, description="X-axis coordinate")
    y_coord: float | None = Field(default=None, description="Y-axis coordinate")
    z_coord: float | None = Field(default=None, description="Z-axis coordinate")

    symbolic_anchors: list[str] | None = Field(
        default=None, description="Symbolic location anchors"
    )
    embedding: list[float] | None = Field(
        default=None, description="Vector embedding used for semantic search"
    )
...
class ProcessedLongTermMemory(BaseModel):
    ...
    emotional_intensity: float | None = Field(
        default=None, ge=0.0, le=1.0, description="Emotional intensity of the memory"
    )
    x_coord: float | None = Field(default=None, description="X-axis coordinate")
    y_coord: float | None = Field(default=None, description="Y-axis coordinate")
    z_coord: float | None = Field(default=None, description="Z-axis coordinate")
