...
from ..utils.pydantic_compat import model_dump_json, model_dump_json_safe
...
class DatabaseManager:
    ...
    def get_connection(self) -> sqlite3.Connection:
        """Public accessor for obtaining a SQLite connection."""
        ...
        with self.get_connection() as conn:
            ...
            cursor.execute(
                ...
                CREATE TABLE IF NOT EXISTS long_term_memory (
                    ...
                    related_memories_json TEXT DEFAULT '[]',
        ...
        try:
            ...
            insert_operation = TransactionOperation(
                ...
                params=[
                    memory_id,
                    chat_id,
                    json.dumps(model_dump_json_safe(memory)),
                    ...
                    memory.z_coord,
                    json.dumps(
                        memory.symbolic_anchors
                        if memory.symbolic_anchors is not None
                        else []
                    ),
                    memory.classification.value,
    ...
    def _calculate_recency_score(self, created_at_str: str) -> float:
        ...
        try:
            ...
            logger.error(f"Recency score calculation error: {e}")
            return 0.0
