"""
SQLite connector for Memoria v1.0
"""
    ...
    def initialize_schema(self, schema_sql: str | None = None):
        ...
        try:
            ...
            if schema_to_use is None:
                ...
                schema_to_use = schema_generator.generate_full_schema()

            if not isinstance(schema_to_use, str) or not schema_to_use.strip():
                raise DatabaseError(
                    "SQLite schema SQL is not available for initialization"
                )

            # Execute schema using transaction
            with self.get_connection() as conn:
                try:
                    conn.executescript(schema_to_use)
                    conn.commit()
