...
try:
    ...
    from tests.utils.test_utils import get_test_model, load_inputs
...
def run_mysql_test_scenario(test_name, conscious_ingest, auto_ingest, test_inputs):
    ...
    mysql_connection = "mysql+mysqlconnector://root:@127.0.0.1:3306/memoria_test"
    ...
    # Initialize Memoria with MySQL backend
    memory = Memoria(**init_kwargs)
    ...
    print("✅ Memoria initialized with MySQL backend")
    ...
    for i, user_input in enumerate(test_inputs, 1):
        try:
            ...
            # Make real LiteLLM API call - this should trigger Memoria callbacks
            response = completion(
                model=get_test_model(),  # Use configured model for testing
                messages=[
                    {
                        "role": "system",
    ...
    resp = memory.db_manager.search_memories("test", namespace=test_namespace)
    search_results = resp.get("results", resp)
