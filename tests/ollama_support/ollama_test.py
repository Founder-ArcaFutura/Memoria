...
if not os.getenv("MEMORIA_RUN_INTEGRATION"):
    pytest.skip(
        "Ollama integration demo – enable MEMORIA_RUN_INTEGRATION=1 to execute.",
...
OLLAMA_MODEL = os.getenv("OLLAMA_MODEL", get_test_model())

print("🦙 Ollama Test with Memoria")
print(f"📍 Server: {OLLAMA_BASE_URL}")
print(f"🤖 Model: {OLLAMA_MODEL}")
print("-" * 50)

# Initialize Memoria with Ollama configuration
ollama_memory = Memoria(
    database_connect="sqlite:///ollama_memory.db",
    conscious_ingest=True,
    auto_ingest=True,
    verbose=True,
    # Configure Memoria to use Ollama for memory agents
