...
This example demonstrates the full power of Memoria's conscious ingestion system:
- Background analysis every 6 hours
- Essential memory promotion
- Intelligent context injection
- Personal identity extraction

Model selection is configured in ``memoria.json``. Update the
``agents.default_model`` field to use a different model.
"""
...
def main():
    print("🧠 Memoria Conscious Ingestion Demo")
    print("=" * 50)
    print()
    ...
    try:
        ...
        print(
            f"📊 Essential conversations: {len(memoria.get_essential_conversations())}"
        )
    except Exception as e:
