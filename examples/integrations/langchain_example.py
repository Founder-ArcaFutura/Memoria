#!/usr/bin/env python3
"""
Lightweight LangChain + Memoria Integration Example
...
- pip install "langchain==0.3.27" langchain-openai python-dotenv
- Set OPENAI_API_KEY in environment or .env file

Usage:
    python langchain_example.py

Model configuration is loaded from ``memoria.json``. Modify the
``agents.default_model`` field to choose a different model.
"""
...
def chat_with_memory(user_input: str) -> str:
    ...
    try:
        ...
        result = agent_executor.invoke(
            {
                "input": user_input,
                "chat_history": [],  # We're using Memoria for persistent memory instead
            }
        )
