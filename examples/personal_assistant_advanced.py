...
Model selection is controlled through ``memoria.json``. Adjust the
``agents.default_model`` field to choose your model.
"""
...
- get_essential_info(): Get core facts about the user"""
    ...
    def search_memory(self, query: str, category: str = None) -> str:
        ...
        try:
            if category:
                results = self.memoria.search_memories_by_category(category, limit=5)
    ...
    def get_essential_info(self) -> str:
        ...
        try:
            essential = self.memoria.get_essential_conversations(limit=10)
            if essential:
                info = []
    ...
    def show_memory_stats(self):
        ...
        print("\\n📊 Memory Statistics:")
        try:
            stats = self.memoria.get_memory_stats()
