#!/usr/bin/env python3
"""
Smart Shopping Experience Demo - Azure AI Foundry + Memoria Integration
...
def load_model_name(default_model: str = "gpt-4o") -> str:
    """Load model name from config.json or AZURE_OPENAI_DEPLOYMENT_NAME env."""
    ...
    if config_path.exists():
        try:
            ...
            pass
    return os.getenv("AZURE_OPENAI_DEPLOYMENT_NAME", default_model)
...
class Config:
    ...
    @classmethod
    def load(cls) -> "Config":
        """Create configuration from config.json or environment variables"""
        load_dotenv()
...
class SmartShoppingAssistant:
    ...
    def _setup_memory(self) -> Memoria:
