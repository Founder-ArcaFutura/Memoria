...
Contains the CrewAI agents and tasks for travel planning with Memoria integration
"""
...
def load_model_name(default_model: str = "gpt-4o") -> str:
    """Load model name from config.json or OPENAI_MODEL env variable."""
...
def create_memory_search_tool(memoria_tool):
    ...
    def memory_search(query: str) -> str:
        ...
        try:
            ...
            if memoria_tool is None:
                return "Memory tool not initialized"

            # Clean the query
            clean_query = query.strip()
            result = memoria_tool.execute(query=clean_query)
            return str(result)
...
class TravelPlannerAgent:
    ...
    def _initialize_memory(self):
        """Initialize Memoria instance and memory tool"""
        try:
            # Create personalized travel memory
            self.travel_memory = Memoria(
    ...
    def get_memory_stats(self) -> dict:
        ...
        try:
            # This would depend on Memoria's actual stats API
