...
def load_model_name(default_model: str = "gpt-5-mini-2025-08-07") -> str:
...
class Researcher:
    """A researcher class that manages Memoria initialization and agent creation"""

    def __init__(self):
        self.memoria = Memoria(
            database_connect="sqlite:///research_memoria.db",
            conscious_ingest=True,  # Working memory
            auto_ingest=True,  # Dynamic search
            verbose=True,
        )
        self.memoria.enable()
        self.memory_tool = create_memory_tool(self.memoria)
        ...
        try:
            ...
            def memory_recording_response(messages, **kwargs):
                ...
                if user_msg or ai_output:
                    try:
                        self.memoria.record_conversation(
    ...
    def _create_research_agent(self):
        """Create a research agent with Memoria memory capabilities and Exa search"""
        agent = Agent(
